ts_model_ui <- function(id) {
  ...
  tagList(reactableOutput(ns("ts-model")))
}

ts_model_mod <- function(id, state) {
  module <- function(input, output, session) {
    state[["ts_yr"]] <- c(2020, 2021)
    state[["ts_var"]] <- "AQI"
    state[["ts_geomean"]] <- TRUE
    state[["ts_trend"]] <- "Linear"
    state[["ts_autocor"]] <- "AR(1)"
    state[["ts_vov"]] <- "GARCH(1,1)"
    ...
    pred <- predict(trend_fit, as.list(data), interval = "confidence")
    ...
    p_trend <- "p-value of trend:" %>%
      paste(round(summary(trend_fit)[["coefficients"]][2, 4], 4))
